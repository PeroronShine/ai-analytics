"""
Analytics Core — обработка результатов анализа, нормализация графиков.
"""
...
@dataclass
class ChartSpec:
    type: str  # bar, line, scatter, histogram, pie, heatmap, box
    title: str
    data: Dict[str, Any]
...
class AnalyticsCore:
    """
    Ядро аналитики: преобразование результатов в структурированный отчёт.
    """
    ...
    def process_agent_result(self, agent_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        Обрабатывает результат работы агента и формирует структурированный отчёт.
        """
        steps = agent_result.get('steps', [])
        final_answer = agent_result.get('final_answer', '')
        
        # Извлечение кода из шагов
        code_snippets = []
        observations = []
    ...
    def _extract_charts_from_steps(self, steps: List[Any]) -> List[Dict[str, Any]]:
        """Извлекает информацию о графиках из шагов агента."""
        charts = []
        # В реальности графики возвращаются как артефакты из sandbox
        # Здесь — структура для дальнейшей обработки
        return charts
    
    def _generate_summary(self, final_answer: str, observations: List[str]) -> str:
        """Генерирует краткое резюме на основе финального ответа и наблюдений."""
        if not final_answer:
    ...
    def _extract_findings(self, text: str) -> List[str]:
        """Извлекает ключевые находки из текста."""
        findings = []
        
        # Ищем маркированные списки, цифры, ключевые фразы
        lines = text.split('\n')
        for line in lines:
            line = line.strip()
        ...
        return findings[:10]  # Максимум 10 находок
    
    def create_plotly_chart(self, spec: ChartSpec) -> go.Figure:
        """
        Создаёт Plotly-график на основе спецификации.
        """
    ...
    def validate_chart_request(self, chart_specs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Валидирует и фильтрует запросы на графики.
        """
        valid = []
